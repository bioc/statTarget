...
### This function provide two.sided Wilcoxon Mann Whitney 
### tests for each pairwise comparison of sample groups.
### file The connection to the data file.
### wmw(file)
...
#perform WMW test for the effective combinations and extract p-values
    ...
    for (q in 1:fin) {
    wilx.pv[q,] <- wilcox.test(I[,q],J[,q], paired=FALSE, 
                               exact=NULL, correct=FALSE, 
                               conf.level=0.95, alternative="two.sided")$p.value
    }
    wmw.ij.pv=paste("WMWTest_pvalues_",
